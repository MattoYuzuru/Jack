...
- home уже ведёт в три живых маршрута: `viewer`, `converter` и `compression`
...
- есть reusable processing-platform: backend отдаёт viewer/converter/compression capability matrix и `platform`-matrix для следующих queued-модулей
...
В текущем срезе реально реализованы девять job type:
...
Контейнерный backend теперь сам ставит `ffmpeg`, `ffprobe`, `ImageMagick`, `Ghostscript`, `potrace` и `libraw`, поэтому `MEDIA_PREVIEW`, `MEDIA_CONVERT`, `IMAGE_CONVERT`, `FILE_COMPRESS` и `OFFICE_CONVERT` работают внутри `docker compose` без внешней подготовки образа.
...
Отдельный `platform` capability scope теперь показывает, как следующие модули (`PDF Toolkit`, `Multi-Format Editor`, `Batch Conversion`, `OCR`, `Office/PDF Conversion`) должны reuse'ить уже существующий processing stack, а не заводить новый browser-heavy runtime.
