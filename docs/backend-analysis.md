...
Вывод: backend уже стал участником продукта и забрал media, heavy imaging и document intelligence домены, но metadata и server-owned capability matrix всё ещё остаются следующими крупными миграциями.
...
- сам всё ещё держит metadata branches и часть capability/scenario решений в браузере
- сам редактирует metadata и собирает export-артефакты
- для legacy media, heavy imaging и document preview уже общается с backend processing API

Это видно по зависимостям и runtime-слою:

- `utif2` для RAW fallback metadata
- `music-metadata-browser`, `exifreader`, `piexifjs`
