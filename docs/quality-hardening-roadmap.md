...
Дата аудита: 12 июля 2026 года. Статус актуализирован 16 июля 2026 года.
...
- visual CI закреплён на официальном Playwright `1.61.1` Linux image: локальная и CI-команды
  используют одинаковое окружение, production build изолирован от host, а report/screenshots/
  traces загружаются как artifacts при падении;
- этап 5 выполняется: Viewer получил abort-aware processing lifecycle, отмену server job при
  ...
  Editor, выделенные presentation module и framework-independent session controller. Следующая
  часть этапа — вынести renderer-компоненты из `ViewerWorkspaceView.vue`;
...
- Markdown contract обновлён до `jack-markdown-1.1.0`: backend формирует единый CSP-защищённый
  `previewDocument` для Viewer и Editor, а GFM-таблицы имеют `scope="col"`, keyboard-scroll
  region, безопасные alignment classes и адаптивные visual checks на 1440/390 px.
- Backend supply-chain gate актуализирован после NVD scan: Spring Boot/Framework, Tomcat,
  Jackson, Log4j, PostgreSQL, SQLite и PDFBox переведены на исправленные версии, неиспользуемый
  Flexmark PDF stack удалён, а H2-база OWASP Dependency-Check сохраняется между CI runner'ами.
