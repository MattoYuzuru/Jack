...
Дата аудита: 12 июля 2026 года. Статус актуализирован 17 июля 2026 года.
...
Статус на 17 июля 2026 года: программа этапов 0–10 закрыта. Исторические находки и исходный план
ниже сохранены как audit trail; они не описывают текущее состояние runtime.

- Viewer использует typed format renderers, единый revision/abort/object-URL lifecycle и bounded
  range-контракты для CSV/TSV, XLS/XLSX/XLSM/ODS и SQLite.
- Editor preview lifecycle выделен из workspace; Converter, Compression, PDF Toolkit, Editor и
  Dev Tools используют общий cancellable task contract, immutable retry inputs и cleanup.
- Backend владеет magic-aware intake, limits, owner-bound upload/job/artifact API, PostgreSQL +
  Flyway state, bounded queue/quotas, TTL cleanup и streaming artifacts.
- HTML/EPUB/SVG/PDF/media paths имеют sanitization, CSP, page/pixel/process/result budgets и
  security postconditions; PDF redaction удаляет text, annotations, attachments и metadata.
- Production containers работают non-root с read-only root; Kubernetes использует bounded
  writable mounts, probes, seccomp, dropped capabilities и default-deny NetworkPolicy.
- CI закрепляет Actions/base images SHA/digest, запускает gitleaks, audit, Dependency-Check,
  SBOM и blocking Trivy High/Critical scan до publish, затем выпускает provenance и Cosign
  signature.
- WCAG/overflow/visual gate выполняется на 320/390/768/1024/1440 px. Initial JS ≤ 150 KiB,
  initial CSS ≤ 40 KiB, lazy JS ≤ 350 KiB и lazy CSS ≤ 80 KiB проверяются автоматически;
  table DOM window ограничен 200 строками, workspace DOM — 2500 nodes.

Текущие deployment prerequisites и остаточные ограничения перечислены в README,
[processing-platform.md](processing-platform.md) и
...
production не выполнялись. Backend dependency SCA теперь входит в CI; полноценный penetration
test в baseline отсутствует и должен быть добавлен отдельным этапом.

## Проверенный baseline

Ниже сначала приведён актуальный completion baseline; последующие audit-факты относятся к
состоянию 12 июля и сохранены для воспроизводимости исходных решений.

- Frontend: lint, format, type-check, 40 suites / 140 unit tests и production budget gate.
- Browser gate: 92 passed, 3 intentionally skipped в pinned Playwright Linux image; axe,
  keyboard, overflow, DOM budget и visual snapshots на пяти ширинах.
- Backend: полный Gradle test suite, OWASP Dependency-Check и CycloneDX SBOM.
- Production images: build/smoke non-root + read-only root; Trivy 0 High/Critical для OS и JAR.
- Infra: `actionlint`, `docker compose config` и `kubectl kustomize` проходят. Offline client
  schema apply не выполняется без доступного Kubernetes API/credentials.

### Исторический baseline аудита
...
- Frontend: 37 test suites, 128 тестов прошли.
- Backend: 19 suites, 59 тестов прошли без failures/errors/skips.
- E2E включает accessibility, responsive-overflow и visual regression на ширинах 320–1440 px;
  каноническая команда — `npm --prefix frontend run test:e2e:linux`.
- Два последовательных канонических E2E-прогона завершились одинаково: 92 passed,
  3 ожидаемо skipped, без flaky/retry.
- `npm audit --audit-level=high`: 0 vulnerabilities.
- OWASP Dependency-Check после обновления runtime graph: 0 vulnerabilities; CI использует
  `NVD_API_KEY`, восстанавливает полную data directory и сохраняет обновлённую базу даже после
  падения vulnerability threshold. Анализ dependency graph выполняется на каждом запуске.
...
Статус 17 июля: SEC-01…SEC-12 закрыты implementation и regression gates. В частности, Markdown
рендерится единым sanitized backend contract; processing имеет ownership/quotas/budgets;
парсеры non-root; XML/SVG/HTML intake hardened; secrets не persisted; metrics не exposed;
artifact download streaming. Таблица ниже описывает первоначальную находку, а не текущую дыру.
